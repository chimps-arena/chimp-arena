...
import { ASTRO_CORP_WALLET, ASTROCHIMPS_COLLECTION, MINT_DELEGATE } from "@/lib/chain/mint-config";

type StepPhase = "idle" | "working" | "error";

/**
 * Two irreversible-by-design, one-time setup steps for the Astrochimps
 * collection, both requiring the Astro Corp wallet specifically (unlike
 * /admin/create-collection, there's no delegate yet to decouple payer from
 * authority here):
 *
 *  1. Lock the Royalties plugin's authority to "None" - permanently
 *     immutable. After this, NOBODY (not a compromised delegate, not even
 *     Astro Corp itself) can ever change the 3%/recipient/ruleSet again.
 *  2. Grant MINT_DELEGATE (a server-only key, see scripts/gen-mint-delegate.mjs)
 *     UpdateDelegate access, so /api/mint/claim can add new assets to the
 *     collection without Astro Corp co-signing every mint. Done AFTER step 1
 *     so this delegate's broader plugin-editing power can never reach the
 *     now-locked royalty.
 */
export function CollectionAuthority() {
  ...
  const refresh = useCallback(async () => {
    ...
    setFetchError(null);
    try {
      const collection = await fetchCollection(umi, publicKey(ASTROCHIMPS_COLLECTION));
    ...
    <div className="flex flex-col gap-4">
      ...
      {royaltiesLocked && delegateGranted && (
        <div className="rounded-xl border border-good/40 bg-good/10 p-4 text-sm text-good">
          Both steps done. New mints can now join the collection with the
          royalty permanently locked in.
        </div>
      )}
    </div>
  );
